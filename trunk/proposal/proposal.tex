...
The SOM is a type of artificial neural network which facilitates the self
organization of training data onto a network of neurons. Traditionally the SOM
uses a rectangular or hexagonal grid of neurons. The topology of these grids
creates a well known problem in SOM called the boundary effect.  Neurons on
...
With the exception of the five platonic solids, distributing points on a
sphere will always result in irregular network topologies \citep{ritter99,
...
regular spherical topology, the main drawback is that the network size, \(N\),
...
tessellation based methods.  A topology which yields a more flexible network
...
spherical topologies that offer greater control over network size. Toward that
...
\caption{The fifty American states and The District of Columbia mapped onto a
...
no substaintive discussion of these irregularities or their effects on SOM
...
should not be overlooked. Particularly for larger SOMs, the ideal network size
...
good feel for how well a SOM fits the training data.  It is a simple
adaptation of a standard SOM diagnostic.  The second diagnostic will be used
to address the reseach question regarding the internal varience and
neighborhood size.  The third diagnostic will address the question concerning
internal variance and topological iregularity.
...
be decomposed to show how well each observation fits the SOM.  For this 
...
each neuron. It would be expected that in traditional SOM neurons closer to
the edge will have higher internal variances. This can be extended to
spherical SOM by comparing the degree of a neuron ($deg(m_i)$ or the number of
