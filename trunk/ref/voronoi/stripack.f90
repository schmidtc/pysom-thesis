...
!  Parameters:
!
!    Input, integer N, the number of nodes in the triangulation.  4 <= N.
!
...
!
  ...
  if ( n1 < 1 .or. nn < n1 .or. nn < 4 .or. lwk < 0 ) then
    ier = 1
    return
  end if
...
!    other than IN1-IN2 unless 0 < IER or LWK = 0.  New arcs to the left of
...
!    and IN2 unless IER > 0.  The data structure has been altered if 4 <= IER.
!
!    Output, integer IER, error indicator:
!    0, if no errors were encountered.
...
!
      if ( left ( x1, y1, z1, x2, y2, z2, x(nl), y(nl), z(nl) ) ) then
        ...
        if ( ( dp2l-dp12*dp1l >= 0.0D+00 .or. &
               dp2r-dp12*dp1r >= 0.0D+00 )  .and. &
             ( dp1l-dp12*dp2l >= 0.0D+00 .or.  &
               dp1r-dp12*dp2r >= 0.0D+00 ) ) then
          go to 6
        end if
!
...
!
!    Workspace, integer NEAR(N), integer NEXT(N), real ( kind = 8 ) DIST(N), 
...
!    latitude, respectively, if IFLAG > 0, or unused dummy parameters if
!    IFLAG < 0.
!
!    Input, integer IFLAG = Nodal coordinate option indicator:
...
!
  ...
  do i = 1, side_max - 1
    if ( count(i) /= 0 ) then
      write ( *, '(i8,i8)' ) i, count(i)
    end if
  end do

  if ( count(side_max) /= 0 ) then
    write ( *, '(i8,i8)' ) side_max, count(side_max)
  end if
...
!
  ...
  if ( abs ( elat ) > 90.0D+00 .or. &
       abs ( elon ) > 180.0D+00 .or. &
       a > 90.0D+00 ) then
    ier = 2
    return
  end if
