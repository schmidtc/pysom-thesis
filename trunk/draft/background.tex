...
This section will desribe the SOM in some detail.  The interested reader is
reffered to \cite{Kohonen2000} for a more in depth discussion of the SOM and
its mathmatical properties.  
In order to explain the SOM we will first define a few key concepts.  

The SOM is composed of two primary compenents, a set of neurons and a topology which defines the connections between them.


The SOM is used to map a high dimmensional input data space onto a low dimmension network of neurons.  Each neuron contains a parametric reference vector, \(m_i\) \cite{Kohonen2000}.  The the parameters of these vecotors are adjusted during a training process such that they begin the model portions of the input space.

 is  vector of parameters
with-in each neuron is used to reference a portion of te input space.  The
training proccess adjusts t



The neurons are trained to represent a high-dimmenisional dataset.  This
dataset can represent a descreate set or a continous input space.  The 
We use a set of training data 


  Each
neuron exists at a fixed point within the network topology.  The neurons do
not move during training, but rather they are tought to repsents different
sections of the 
A neuron represents a location both with in the SOM and within the
higher-dimmension data space.
