...
int main(int argc, char *argv[])
{
  ...
  //v_array<vector> v = make_same(10000);  
  int k = atoi(argv[1]);
  v_array<vector> v = parse_points(argv[2]);
  ...
  node<vector> top = batch_create(v);
  timeval created;
  ...
  //qsort(v.elements, v.index,sizeof(vector), compare);
  int thresh=MAXINT;
  ...
  v_array<v_array<vector> > brute_neighbors;
  for (int i=0; i < res.index  && i < thresh; i++) {
    vector this_point = res[i][0];
    float upper_dist[k];
    vector min_points[k];
    for (int j=0; j<k; j++)
      {
	upper_dist[j] = MAXFLOAT;
	min_points[k] = this_point;
      }
  ...
  for (int i=0; i < brute_neighbors.index; i++) {
    vector this_point = brute_neighbors[i][0];
    for (int j = 1; j < brute_neighbors[i].index; j++)
      {
	int flag = 0;
	vector this_neighbor = brute_neighbors[i][j];
