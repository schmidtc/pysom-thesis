...
cautious when using methods that limit the control of network size.  The size of
the network represents a continuum along which data reduction occurs.
\cite{skupin07} demonstrate this when they use the same data to train two SOMs
of different sizes.  In the three-by-three case the neurons act as containers clustering
similar states, while in the twenty-by-twenty case the observations are mapped in a way
that represents their relationship to other states in higher dimensions.  As
shown in Figure \ref{fig:nSize}, methods for arranging an arbitrary number of
points on a sphere provide a much higher degree of flexibility when choosing a
network size.
