...
import os
from pysom import voronoi
import delaunay
from StringIO import StringIO
""" Produces a NetworkX topology """
def rakhmanov(n):
    """ Returns a rakhmanov distribution of n points on the surface of a unit sphere """
    try:
        assert n != 1
    except AssertionError:
        print "n must be a positive integer > 1"
        return None
    points = []
...
def nxRakhmanov(n):
    """ Build the network X object """
    grid = map(toXYZ,rakhmanov(n))
    xyzFile = os.tempnam()
    o = open(xyzFile,'w')
    for x,y,z in grid:
        o.write("%f %f %f\n"%(x,y,z))
    ...
    results = voronoi.build(xyzFile,delXYZ=True)
    s = StringIO(results[0])
    return delaunay.parseDelaunay(s)
